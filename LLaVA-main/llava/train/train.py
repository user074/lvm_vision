...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['mm_projector', 'dino_mm_projector', 'vision_tower', 'vision_resampler']
...
def train(attn_implementation=None):
    ...
    if training_args.bits in [4, 8]:
        ...
        bnb_model_from_pretrained_args.update(dict(
            ...
            quantization_config=BitsAndBytesConfig(
                load_in_4bit=training_args.bits == 4,
                load_in_8bit=training_args.bits == 8,
                llm_int8_skip_modules=["mm_projector", "dino_mm_projector"],
    ...
    if training_args.lora_enable:
        ...
        if training_args.local_rank == 0 or training_args.local_rank == -1:
            print(f"save models to {training_args.output_dir} ")
            model.config.save_pretrained(training_args.output_dir)
            model.save_pretrained(training_args.output_dir, state_dict=state_dict)
            torch.save(non_lora_state_dict, os.path.join(training_args.output_dir, 'non_lora_trainables.bin'))
    trainer.save_state()
    print('trainer save state')
