...
from .multimodal_projector.builder import build_vision_projector, build_vision_adopter
...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            self.fusion_adapter = build_vision_adopter(config)
    ...
    def get_vision_adaptor(self):
        return getattr(self, 'fusion_adapter', None)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        pretrain_fusion_adapter = model_args.pretrain_fusion_adapter
        ...
        if getattr(self, 'fusion_adapter', None) is None:
            self.fusion_adapter = build_vision_adopter(self.config)
        ...
        if pretrain_fusion_adapter is not None:
            ...
            self.fusion_adapter.load_state_dict(get_w(fusion_adapter_weights, 'fusion_adapter'))
...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_vision_adaptor(self):
        return self.get_model().get_vision_adaptor()
    ...
    def prepare_inputs_labels_for_multimodal_withdino(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            while image_token_indices.numel() > 0:
                ...
                cur_image_features_fusion = fusion_features[cur_image_idx]
                ...
                if getattr(self.config, 'tune_mm_mlp_adapter', False) and getattr(self.config, 'mm_use_im_start_end', False):
                    ...
                    num_patches, clip_dim = cur_image_features_fusion.shape

                    clip_dtype = cur_image_features_fusion.dtype

                    # Interleave features
                    # merged_features = torch.empty(num_patches, clip_dim, dtype = clip_dtype)
                    merged_features = cur_image_features_fusion
                    ...
                    if labels is not None:
                        ...
                        cur_new_labels.append(torch.full((cur_image_features_fusion.shape[0],), IGNORE_INDEX, device=labels.device, dtype=labels.dtype))
